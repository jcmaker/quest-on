...
import { createAuthPage } from "../helpers/auth-context";
...
test.describe("Grade to Report — Cross-Role Flow", () => {
  ...
  test("instructor grades submissions, student sees scores on report page", async ({
    ...
    const studentContext = await browser.newContext({ baseURL: url });
    const instructorContext = await browser.newContext({ baseURL: url });
    const instructorPage = await createAuthPage(
      instructorContext,
      TEST_INSTRUCTOR,
      url
    );
    const studentPage = await createAuthPage(studentContext, TEST_STUDENT, url);
    ...
    try {
      ...
      await expect(
        instructorPage.getByRole("heading", { name: /학생 채점/ }),
      ...
      if (await q1Nav.isVisible({ timeout: TIMEOUTS.ELEMENT_VISIBLE }).catch(() => false)) {
        await q1Nav.click();
