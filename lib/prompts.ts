...
- 수업 자료에 근거가 없는 질문이라도, 시나리오와 관련된 질문이면 **교수자로서 가상의 상황에 맞는 합리적인 데이터를 생성하여 답변**합니다. 당신이 가상 시나리오를 설계한 교수이므로, 시나리오 세계관 안의 정보는 자유롭게 설정할 수 있습니다.
- 수업 자료와 **직접 충돌하는** 수치나 사실을 만들어내지는 마세요. 자료가 제공된 부분은 자료를 우선합니다.
- 시나리오와 전혀 무관한 질문(예: 개인 정보, 시험과 관계없는 일반 상식)에만 "시나리오 범위 밖의 내용입니다"라고 답변합니다.
`.trim();
}
...
- 시나리오와 관련된 정보 질문에는, 수업 자료에 없더라도 가상 상황에 맞는 합리적인 데이터를 생성하여 답변한다. 단, 수업 자료와 직접 충돌하는 내용은 만들지 않는다.
`.trim();
}
...
출력:
{
  "questions": [
    {
      "text": "<h3>사례: 그린테크 에너지의 태양광 패널 가격 전략</h3><p>그린테크 에너지는 국내 태양광 패널 시장의 선도 기업입니다. 주력 제품 'GT-500'은 프리미엄 가격대에 포지셔닝되어 있으며, 최근 정부가 태양광 패널 설치 보조금을 대폭 축소하고 중국산 저가 패널의 수입이 급증하면서 경영진은 가격 전략 재검토를 논의 중입니다.</p><ol><li>보조금 축소와 저가 경쟁재 등장이 GT-500 수요에 미치는 영향을 수요의 가격탄력성 개념을 활용하여 분석하시오. 태양광 패널이 필수재인지 사치재인지에 대한 본인의 판단을 근거와 함께 제시하시오.</li><li>위 분석을 바탕으로 그린테크의 CEO로서 GT-500의 가격 전략(인하 vs 유지)을 결정하시오. AI에게 필요한 수치를 탐색하고, 예상 수익을 비교하여 근거 있는 전략을 제시하시오.</li></ol>",
      "type": "essay",
      "rubric": [
        {
          "evaluationArea": "경제학 개념 적용",
          "detailedCriteria": "수요의 가격탄력성 등 핵심 개념을 시나리오에 정확하게 적용하였는가"
        },
        {
          "evaluationArea": "전략적 사고와 논증",
          "detailedCriteria": "수치 데이터를 활용한 비교 분석과 트레이드오프를 고려하여 근거 있는 전략을 제시하였는가"
        },
        {
