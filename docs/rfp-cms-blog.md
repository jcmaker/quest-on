...
- 카테고리/태그 시스템 (필터링 가능)
- 반응형 디자인 (모바일 필수)
- 작성자(author) 프로필 — 이름, 사진, 간단한 소개 (게스트 라이터 확장 대비)
...
**CTA (Call to Action)**
- 글 내 또는 글 하단에 **회원가입 유도** CTA
- **뉴스레터 구독** CTA (이메일 캡처) — 구독 폼 UI + 이메일 저장. 실제 발송 연동(Resend 등)은 후순위

**블로그 접근 경로 (네비게이션)**
- PublicHeader (비로그인) — Blog 링크 노출
- Instructor/Admin 네비게이션 — Blog 링크 노출
- **Student 네비게이션에서는 제외**
- Footer — Blog 링크 노출
...
- 목차(TOC) 자동 생성
- 콘텐츠 게이팅 — 특정 콘텐츠(백서, 리포트 PDF 등)를 이메일 입력 후 다운로드 가능하게. 리드 캡처 용도
...
| **C. Headless CMS 연동** | Sanity, Contentful 등 외부 CMS | 에디터 UX 즉시 확보, 비개발자 친화 | 외부 의존성, API 비용 발생 가능 |
| **D. Payload CMS** | Next.js 앱 안에서 직접 돌아가는 오픈소스 CMS | 단일 배포, 우리 Supabase Postgres 재활용 가능, TypeScript 네이티브 | 비교적 신생 (생태계 작음) |
...
- [ ] URL 루트 경로 — `/blog` vs `/resources` (확장성 관점에서 판단)
- [ ] 다국어 지원 여부 및 방식 — 한국어+영어 둘 다 가능하면 좋겠지만, 공수 대비 효용이 낮다고 판단되면 한국어만으로 시작해도 됨

**단, 아래는 지켜주세요:**
- URL 패턴은 `quest-on.app/[루트]/[slug]` (루트가 `/blog`든 `/resources`든 flat 구조 유지)
- SEO 메타데이터는 글별로 커스터마이징 가능해야 함
- 기존 Quest-On 디자인 시스템 / 컴포넌트 재활용
- 글 상태(draft/published)는 반드시 구분
- 작성자(author) 프로필 — 글 내부에 이름 + 사진 표시 필수
- 블로그 네비게이션은 학생 뷰에서 제외 (비로그인, Instructor, Admin에만 노출)
