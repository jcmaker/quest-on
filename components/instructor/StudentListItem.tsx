...
function GradeStatusBadge({ gradeType }: { gradeType?: InstructorStudent["gradeType"] }) {
  if (!gradeType) return null;

  const config = {
    ai_graded: { label: "AI 채점", className: "bg-blue-100 text-blue-700 border-blue-200" },
    manually_graded: { label: "최종 확정", className: "bg-green-100 text-green-700 border-green-200" },
    pending: { label: "채점 대기", className: "bg-orange-100 text-orange-700 border-orange-200" },
  } as const;

  const { label, className } = config[gradeType];

  return (
    <Badge variant="outline" className={`text-xs font-normal flex-shrink-0 ${className}`}>
      {label}
    </Badge>
  );
}
...
export function StudentListItem({
  ...
  return (
    <Collapsible open={expanded} onOpenChange={setExpanded}>
      <div className="hover:bg-muted/50 transition-colors overflow-hidden">
          ...
          <div className="flex items-start gap-4 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <GradeStatusBadge gradeType={student.gradeType} />
              </div>
        ...
        <CollapsibleContent>
          {student.aiComment && (
            <div className="px-4 pb-4 pt-0">
              <div className="ml-14 p-3 bg-muted/30 rounded-lg border text-sm">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="font-medium text-xs text-muted-foreground uppercase tracking-wide">AI 채점 피드백</span>
                </div>
